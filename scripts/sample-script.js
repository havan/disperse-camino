...
const values = [getTokenValue(10), getTokenValue(20), getTokenValue(30), getTokenValue(40), getTokenValue(50)];
...
async function main() {
    ...
    const approve = await alice.approve(disperse.address, BigNumber.from(150).mul(ETHER));

    const transfer = await disperse.disperseToken(alice.address, recipients, values);
